        ...
        for compression in compression_values:
            ...
            with PdfPages(pdf_path_combined) as pdf_combined:
                ...
                for i, method in enumerate(['DCT', 'DFT', 'DWT', 'PAA', 'MINE']):
                    file_name_r2 = f'{method}_stride({stride})_compression({compression} - SILHOUETTES)'
                    file_path_r2 = os.path.join(base_path, f'stride({stride})', f'compression({compression})', f'{file_name_r2}.txt')

                    file_name_rmse = f'{method}_stride({stride})_compression({compression} - ARIs)'
                ...
                # Add R2 title
                fig_combined.text(0.5, 0.98, f'Silhouette Scores with 95% Confidence Interval // W={stride}, C={compression}', fontsize=12, ha='center')

                # Add RMSE title
                fig_combined.text(0.5, 0.48, f'ARI scores with 95% Confidence Interval // W={stride}, C={compression}', fontsize=12, ha='center')
...
base_path = r'C:\Users\tony7\Desktop\Updated Measurements\1.2DBSCAN-INTEL\DBSCAN Lights Measurements(eps = 0.4 )'
